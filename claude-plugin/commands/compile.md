...
0.5. **Topic schema check**: If `<wiki-root>/schema.md` exists, read it in full before proceeding. This does not change the `category` vocabulary (still `concept`/`topic`/`reference`, per step 4) — it's structural guidance for the same step: if it declares a cardinality rule (e.g. "one article per mechanism, not per raw file — consolidate multiple instances of the same thing"), an Include/Exclude scoping rule, or specific near-duplicate pairs to keep separate rather than merge, apply that when deciding in steps 3-4 whether a raw source is worth compiling at all, and whether a concept warrants its own article, a fold-in, or a stays-separate decision. If `schema_state: strict`, treat any article that would deviate from the declared guidance as a stop-and-ask, not a judgment call. If no `schema.md` exists, this step is a no-op.
