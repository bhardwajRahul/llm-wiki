...
0.5. **Topic schema check**: If `<wiki-root>/schema.md` exists, read it in full before proceeding. This does not change the `category` vocabulary (still `concept`/`topic`/`reference`, per step 4) — it's structural guidance for the same step: if it declares a cardinality rule (e.g. "one article per mechanism, not per raw file — consolidate multiple instances of the same thing"), an Include/Exclude scoping rule, or specific concept pairs to keep as separate articles rather than folding one into the other, apply that when planning in step 4 whether a raw source is worth compiling at all, and whether a concept gets its own article, gets folded into an existing one, or is just mentioned within one. If `schema_state: strict`, warn when an article would deviate from the declared guidance rather than silently defaulting to your own judgment — per the plugin's strict-mode contract, never auto-rewrite an already-compiled article to force compliance. If no `schema.md` exists, this step is a no-op.
