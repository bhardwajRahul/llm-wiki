...
set +e
add_output="$("$CLI" adapter add "$adapter" --read-root "$inputs" --write-root "$outputs" --json 2>&1)"
add_rc=$?
set -e
...
if python3 -c 'import json,sys; d=json.load(sys.stdin); assert len(d["adapters"]) == 1; assert d["adapters"][0]["id"] == "fixture-private"' <<<"$list_output" \
  && python3 -c 'import json,sys; d=json.load(sys.stdin); assert d["id"] == "fixture-private"; assert d["env_allow"] == []; assert d["command"][0].endswith("/bin/adapter")' <<<"$show_output"; then
  log_pass "list and show preserve executable symlinks without storing credential values"
else
  log_fail "list and show preserve executable symlinks without storing credential values" "$list_output $show_output"
fi
